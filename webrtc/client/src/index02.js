...
socket.on('newProducer', async ({ producerId, clientId, kind }) => {
    ...
    socket.emit('consume', { producerId, kind : kind, rtpCapabilities: device.rtpCapabilities},async (params) => {
        ...
        try {
            const consumer = await recvTransport.consume({id : params.id,  producerId : params.producerId,kind : params.kind, rtpParameters: params.rtpCapabilities});
...
document.getElementById('toggleVideo').addEventListener('click', () => {
    ...
    document.getElementById('toggleVideo').innerText = isVideoEnabled ? '비디오 끄기' : '비디오 켜기';
    console.log(`비디오 상태 변경: ${isVideoEnabled ? '켜짐' : '꺼짐'}`);
});

document.getElementById('toggleAudio').addEventListener('click', () => {
    isAudioEnabled = !isAudioEnabled;
    localStream.getAudioTracks().forEach(track => track.enabled = isAudioEnabled);
    document.getElementById('toggleAudio').innerText = isAudioEnabled ? '오디오 끄기' : '오디오 켜기';
    console.log(`오디오 상태 변경: ${isAudioEnabled ? '켜짐' : '꺼짐'}`);
});
